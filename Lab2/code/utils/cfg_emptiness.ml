...
let rec back_path (dico : (variable, sentential_form) Hashtbl.t) (a : alpha) : (meth list) =
  match a with 
    |Terminal t -> [t]
    |Var v -> let l = Hashtbl.find dico v in 
                List.flatten (List.map (fun a -> back_path dico a) l)

let is_generating (c:cfg) (v:variable) : (meth list) option = 
...
let is_empty (c:cfg) : meth list option = is_generating c Start